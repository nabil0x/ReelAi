...
def main() -> None:
    ...
    track_masks = build_frame_masks(w, h, fps, total)

    top_px = int(h * CAPTION_TOP_FRAC)
    bot_px = int(h * CAPTION_BOT_FRAC)
    font = ImageFont.truetype(font_path, 44)
    per_line = max(1, total // max(1, len(lines)))
    anchors = build_caption_anchors(h, fps, total)
    ...
    for ci in range(0, total, CHUNK_SIZE):
        ...
        for j, f in enumerate(chunk):
            ...
            if track_masks is not None and gi < len(track_masks):
                m = track_masks[gi]
            else:
            ...
            cap_line = lines[min(gi // per_line, len(lines) - 1)][:42]
            bb = draw.textbbox((0, 0), cap_line, font=font)
            tw, th = bb[2] + 40, bb[3] + 24
            if anchors[gi] == "bottom":
                by = h - bot_margin - th
                draw.rectangle([(w - tw) // 2, by, (w + tw) // 2, by + th],
                               fill="white")
                draw.text(((w - tw) // 2 + 20, by + 12), cap_line, font=font,
                          fill="black")
            else:
                draw.rectangle(
                    [(w - tw) // 2, top_margin, (w + tw) // 2, top_margin + th],
                    fill="white",
                )
                draw.text(((w - tw) // 2 + 20, top_margin + 12), cap_line,
